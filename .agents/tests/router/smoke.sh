...
EOF
	...
	grep -Fq 'Target shorthand: omitted target means current host; bare `HOST` means `ssh:HOST`; `ALL` expands from the active home policy.' "$help"
	grep -Fq 'When traversing `ALL`, skip hosts that fail a bounded noninteractive reachability check and continue with reachable hosts.' "$help"
	...
	grep -Fq 'Prompt: `/tilde status [HOST|ssh:<host>|ALL]`' "$status_help"
	...
	grep -Fq 'Prompt: `/tilde update [HOST|ssh:<host>|ALL] [full] [dry-run|plan-only]`' "$update_help"
