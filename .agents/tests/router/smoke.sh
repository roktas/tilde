...
assert_fallback_handoff_command() {
	...
	[[ $command == *'r="~/.agents/skills/tilde"'* ]] || abort "handoff command should target the default remote runtime root"
...
EOF
	...
	grep -Fq 'Successful Tilde-generated refresh/update plans may advance `applied` anchors; report target HEAD separately from applied anchors.' "$help"
	grep -Fq 'Remote start route: run `"$TILDE" preflight remote HOST --format json` before target status, plan, or apply to classify bootstrap, repository backend, runtime, and sudo cleanup state.' "$help"
	grep -Fq 'After remote runtime freshness is verified, read target `tilde status --format shell` into a temp file and use `tilde_public_repo`, `tilde_private_repo`, and `tilde_update_mode` as plan bindings.' "$help"
	grep -Fq 'Bind remote plan repository paths to variables from target status shell output; do not retype host-convention paths in `plan --repo` commands.' "$help"
	...
	grep -Fq 'Dropbox-backed runtime steady state is `~/.agents/skills/tilde -> ~/Dropbox/tilde`; cloned runtime checkouts are provisional bootstrap/freshness fallback until the Dropbox checkout is present, clean, executable, and current.' "$help"
